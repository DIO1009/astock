...
func TestStockInfoFromRawUsesConfirmedClistReturnMapping(t *testing.T) {
	raw := rawStock{
		F12:  "600503",
		F14:  "华丽家族",
		F13:  num("1"),
	...
	if !almostEqual(stock.Ret10d, 1.17) {
		t.Fatalf("Ret10d = %v, want %v", stock.Ret10d, 1.17)
	}
	if !almostEqual(stock.Ret20d, 13.04) {
		t.Fatalf("Ret20d = %v, want %v", stock.Ret20d, 13.04)
	}
}
...
func TestStockInfoFromRawRejectsNewStockNames(t *testing.T) {
	tests := []rawStock{
		validRawStock("603435", "C嘉德"),
		validRawStock("001234", "N新股"),
	}

	for _, raw := range tests {
		if _, ok := stockInfoFromRaw(raw); ok {
			t.Fatalf("stockInfoFromRaw accepted new-stock name %q", raw.F14)
		}
	}
}
...
func validRawStock(symbol, name string) rawStock {
	return rawStock{
		F12:  symbol,
		F14:  name,
		F13:  num("1"),
