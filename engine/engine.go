...
func (e *Engine) processTick() {
	...
	if e.safetyGuard != nil {
		e.safetyGuard.AdvanceTick()
	}
	tradeDay := e.tradeDaySeq()
	if e.posMgr != nil {
		e.posMgr.AdvanceTradeDay(tradeDay)
	}
	...
	if e.cfg.IndexSymbol != "" {
		indexQuote = allQuotes[e.cfg.IndexSymbol]
	}

	dataCheckOK := true
	if e.dataChecker != nil {
		result := e.dataChecker.Check(stockQuotes, indexQuote)
		for _, fs := range result.Factors {
			log.Printf("[DataCheck] 因子统计  %s", fs)
		}
	...
	positions := e.posMgr.AllPositions()
	if e.cfg.LogRank && e.portMgr != nil {
		e.logPortfolioStats(e.portMgr.Stats(positions), positions, stockQuotes)
	}
		...
		for _, pos := range positions {
			...
			if trade != nil {
				...
				if reg, ok := e.alphaEng.(core.StrategyRegistry); ok {
					reg.RecordSell(pos.Symbol, pnlPct)
				}
			}
		}
	}
	...
	for _, pos := range positions {
		...
		if exitType == "HOLD" || pos.SellableQty <= 0 {
			continue
		}
	...
	if e.alphaEng != nil {
		signals = e.alphaEng.Rank(stockQuotes)
	}
	if e.adjuster != nil {
		signals, _ = e.adjuster.Adjust(signals)
	}
	if e.cfg.LogRank {
		e.logRanking(signals, nil)
	}
	...
	if e.stabilizer != nil {
		...
		if e.cfg.LogRank {
			e.logStability(stabilityCounts, stableSignals)
		}
	}
	if e.dashboard != nil {
		if dw, ok := e.dashboard.(interface{ SetWatchList(map[string]int) }); ok {
			dw.SetWatchList(stabilityCounts)
		}
		if dw, ok := e.dashboard.(interface{ SetSignalCache([]core.Signal) }); ok {
			dw.SetSignalCache(signals)
		}
	}
	...
	if e.safetyGuard != nil && !e.safetyGuard.AllowOpen() {
	...
	if marketAllows && safetyAllowsOpen && dataCheckOK && e.portMgr != nil && e.portDecision != nil {
		...
		if e.portMgr.CanOpenPosition(positions) {
			...
			if (mktState == core.MarketOscillate || mktState == core.MarketUptrend) && regimeMinScore > 0 {
				...
				for _, sig := range stableSignals {
					if sig.Score >= regimeMinScore {
			...
			for _, order := range orders {
				if e.posMgr.HasPosition(order.Symbol) || !e.portMgr.CanOpenPosition(e.posMgr.AllPositions()) {
					continue
				}
				if e.execCtrl != nil {
					if q := stockQuotes[order.Symbol]; q != nil {
						if allowed, reason := e.execCtrl.AllowBuy(order.Symbol, q.Price); !allowed {
							log.Printf("  🚫 [ExecCtrl] BUY %-8s BLOCKED  %s", order.Symbol, reason)
							continue
						}
					}
				}
				...
				if trade != nil {
					if e.execCtrl != nil {
						if q := stockQuotes[order.Symbol]; q != nil {
							e.execCtrl.RecordBuy(order.Symbol, q.Price)
						}
					}
					if e.perfTracker != nil {
						e.perfTracker.OnBuy(trade)
					}
					if reg, ok := e.alphaEng.(core.StrategyRegistry); ok {
						if sig, found := sigMap[order.Symbol]; found {
							reg.RecordBuy(order.Symbol, sig.Breakdown)
						}
					}
				}
			}
	...
	if e.cfg.LogRank {
		e.logPnL(stockQuotes)
	}
	if e.perfTracker != nil {
		positions = e.posMgr.AllPositions()
		equity := e.perfTracker.Cash()
		for _, pos := range positions {
			if q := stockQuotes[pos.Symbol]; q != nil {
				equity += float64(pos.Quantity) * q.Price
			} else {
				equity += float64(pos.Quantity) * pos.AvgPrice
			}
		...
		if e.monitor != nil {
			e.monitor.Update(equity, e.perfTracker.Report(), positions)
		}
		if e.dashboard != nil {
			e.dashboard.OnTick(equity, e.perfTracker.Report(), positions, stockQuotes)
		}
	}
	if e.adaptiveOpt != nil && e.perfTracker != nil {
		params := e.adaptiveOpt.Params(e.perfTracker.Report())
		if setter, ok := e.portMgr.(core.MaxTotalPctSetter); ok {
			setter.SetMaxTotalPct(params.MaxTotalPct)
		}
		if setter, ok := e.portDecision.(core.BuyThresholdSetter); ok {
			setter.SetBuyThreshold(params.BuyThreshold)
		}
		if params.LogReason != "" && (params.MaxTotalPct != e.lastAdaptiveParams.MaxTotalPct || params.BuyThreshold != e.lastAdaptiveParams.BuyThreshold) {
			log.Println(params.LogReason)
		}
		e.lastAdaptiveParams = params
	}
	if reg, ok := e.alphaEng.(core.StrategyRegistry); ok && e.tickCount%20 == 0 {
		e.logWeightSnapshot(reg.WeightSnapshot())
	}
}
...
func (e *Engine) processRotation(signals []core.Signal, candidateSignals []core.Signal, stockQuotes map[string]*core.Quote, tradeDay int64) {
	if e.rotationPol == nil || e.posMgr == nil {
		return
	}
	ranks := make(map[string]rotation.RankInfo, len(signals))
	for i, sig := range signals {
		ranks[sig.Symbol] = rotation.RankInfo{Symbol: sig.Symbol, Rank: i + 1, Score: sig.Score}
	}
	...
	for _, pos := range e.posMgr.AllPositions() {
		...
		if rank.Symbol == "" {
			rank = rotation.RankInfo{Symbol: pos.Symbol, Rank: rotation.MissingRank}
		}
		pnlPct := (q.Price - pos.AvgPrice) / pos.AvgPrice * 100
		...
		if !decision.Rotate || pos.SellableQty <= 0 {
			continue
		}
		if e.execCtrl != nil && !e.execCtrl.AllowSell(pos.Symbol, "ROTATION") {
			continue
		}
		holdTicks := 0
		if e.execCtrl != nil {
		...
		if trade != nil {
			if e.execCtrl != nil {
				e.execCtrl.RecordSell(pos.Symbol, q.Price, "ROTATION")
			}
...
func (e *Engine) bestRotationCandidate(signals []core.Signal, ranks map[string]rotation.RankInfo, oldSymbol string, reserved map[string]bool) *rotation.CandidateInfo {
	for i, sig := range signals {
		if sig.Symbol == oldSymbol || reserved[sig.Symbol] || e.posMgr.HasPosition(sig.Symbol) {
			continue
		}
		rank := i + 1
		if info, ok := ranks[sig.Symbol]; ok {
			rank = info.Rank
		}
...
func (e *Engine) executeOrder(order core.Order, quotes map[string]*core.Quote) *core.Trade {
	if e.executor == nil || e.posMgr == nil || e.tradeLogger == nil {
		return nil
	}
	...
	if err != nil {
		log.Printf("[Engine] order rejected %s %s: %v", order.Side, order.Symbol, err)
		return nil
	}
...
func (e *Engine) maybeReview() {
	...
	if now.Weekday() == e.cfg.ReviewWeekday && now.Hour() == e.cfg.ReviewHour && week != e.lastReviewWk {
		...
		if err := e.reviewer.Review(); err != nil {
			log.Printf("[Engine] weekly review error: %v", err)
		}
	}
}
...
func (e *Engine) logPortfolioStats(s core.PortfolioStats, positions []core.Position, quotes map[string]*core.Quote) {
	...
	for _, p := range positions {
		...
		unrlzd := 0.0
		if p.AvgPrice != 0 {
			unrlzd = (nowPrice - p.AvgPrice) / p.AvgPrice * 100
		}
...
func (e *Engine) logRanking(signals []core.Signal, dampened map[string]int) {
	...
	for i, sig := range signals {
		...
		for name, score := range sig.Breakdown {
			parts = append(parts, fmt.Sprintf("%s=%+.3f", name, score))
		}
		sort.Strings(parts)
		dampenTag := ""
		if streak, ok := dampened[sig.Symbol]; ok {
			dampenTag = fmt.Sprintf("  🔻streak=%d", streak)
		}
...
func (e *Engine) logStability(counts map[string]int, stableSignals []core.Signal) {
	...
	for _, sym := range syms {
		...
		if stableSet[sym] {
			status = fmt.Sprintf("✅ STABLE(%d) → BUY eligible", cnt)
		}
...
func (e *Engine) logPnL(quotes map[string]*core.Quote) {
	...
	for _, p := range positions {
		...
		maxPnl := 0.0
		if p.AvgPrice != 0 {
			unrlzd = (q.Price - p.AvgPrice) / p.AvgPrice * 100
			maxPnl = (p.HighestPrice - p.AvgPrice) / p.AvgPrice * 100
		}
